...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/pockyyang/crack-soga-v2ray/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/pockyyang/crack-soga-v2ray/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/soga -N --no-check-certificate https://github.com/pockyyang/crack-soga-v2ray/raw/master/soga.sh
    if [[ $? != 0 ]]; then
        echo ""
